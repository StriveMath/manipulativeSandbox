...
function Disk({ place, index, animate, highlight }) {
  return (
    <div
      className={`flex h-7 w-7 items-center justify-center rounded-full border-2 border-white ${place.color} text-[8px] font-bold text-white shadow-sm ${
...
function RegroupGhostDisk({ place, className = '', size = 'large', style }) {
  const sizeClass = size === 'small' ? 'h-8 w-8 text-[9px]' : 'h-14 w-14 text-sm'
        ...
        {places.map((place, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="mt-2 flex min-h-0 flex-1 content-start items-start justify-center overflow-hidden rounded border border-white/80 bg-white/70 p-2">
